...
We Remove unwanted water molecules, save any ligands attached and clean up the introductory information within the file

##################################################################################
Original author: Dr. Huan Wang
Adaptation: Connor Sicheri
##################################################################################

# How to run this script:
python PDB_cleaner/pdb_cleaner.py pdbs clean_pdbs true
"""
...
def pdb_reader(filename):
    ...
    if pdb_df.shape[0] > terminal_id[-1]:
        ...
        ligand = ligand[ligand['ResName'] != "HOH"]
...
def non_std_residues(filename, pdb_df):
    ...
    pdb_df['ResName'] = pdb_df['ResName'].astype(str)
    print(pdb_df['ResName'].to_string())
...
def main(path, keep, hydrogen, report):
    ...
    for i, f in enumerate(pdbfiles):
        ...
        if "NMR" in method:
            ...
            nonstdRes = False #non_std_residues(filename, pdb_df)    # return a tuple
            if nonstdRes:
            ...
            if report == "c":
                ...
                if (save_ligands == "True" or save_ligands == "true" or save_ligands == "t"):
                    ligand_path=outputpath + "/" + f[:4] + "_ligands.pdb"
                    save_cleaned_PDB(ligand_path, ligand)
    ...
    return outputf, ligand_path


################################## main #######################################
if __name__ == "__main__":
    pdb_path, ligand_path = main(filepath, k_option, remove_H, r_option)
    print(ligand_path)
    print(pdb_path)
